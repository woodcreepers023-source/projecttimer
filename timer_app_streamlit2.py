...
# ------------------- INSTAKILL PAGE -------------------
    ...
    if not st.session_state.auth:
        ...
        # ------------------- Toast system (2–3 seconds) -------------------
        st.session_state.setdefault("ik_toast", None)

        def show_instakill_toast():
            toast = st.session_state.get("ik_toast")
            if not toast:
                return
            ...
            # keep rerunning while toast is visible (every 0.5 sec)
        ...
        for start in range(0, len(timers_sorted), CARDS_PER_ROW):
            ...
            for j in range(CARDS_PER_ROW):
                with cols[j]:
                    ...
                    # ✅ Killed Now: sets LAST TIME to NOW and saves
                    if st.button("💀 Killed Now", key=f"ik_killednow_{t.name}", use_container_width=True):
                        ...
                        # Update object in session_state
        ...
        # ✅ Put the toast at the bottom (like your screenshot)
        show_instakill_toast()
